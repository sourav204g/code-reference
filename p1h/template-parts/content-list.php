				...
				<div class="ptagq">
                    ...
                    <?php else: ?>
                     ...
                     <?php else: ?>
						...
						name="<?php echo 'addon_subservice_list_' . $outerKey . '_' . $innerKey; ?>" 
						type="radio" 
						value="<?php echo esc_attr($handyman_so_list_option['handyman_so_list_options_label']) . '|' . $handyman_so_list_option['handyman_so_list_options_labour_minutes'] . '|' . $suboptionlistMaterial; ?>" data-premium="<?php echo get_field('handyman_product_premium', $post->ID); ?>" data-discount="<?php echo get_field('handyman_product_discount', $post->ID); ?>">
					...
					<label for="<?php echo $outerKey . '|' . $innerKey . '|' . $subkey . '|' . $subsubkey; ?>">
						...
						if (get_field('handyman_product_premium', $post->ID)) {
						  ...
						  $ListPremium = ( $suboptionlistCost * get_field('handyman_product_premium', $post->ID) )/100;
						...
						if (get_field('handyman_product_discount', $post->ID)) {
						  ...
						  $ListDiscount = ($suboptionlistCost * get_field('handyman_product_discount', $post->ID))/100;
