...
jQuery(document).ready(function($) {
	...
	$('.hnd-search-close').click(function(){
		$('#company_works_at').val('');
		$('#company_works_at1').val('');
		$('span.close-btn.hnd-search-close').css('display', 'none');
	...
	textInput.onkeyup = function (e) {
	    ...
	    if (textInput.value != '') {
	    	...
	    	$('span.close-btn.hnd-search-close').css('display', 'inline');
		    timeout = setTimeout(function () {
		    	$('.custom-autocomplete-suggestions').css('display', 'block');
		    	$('.custom-autocomplete-suggestions').find('ul').html('<li>Loading..</li>');
					...
					success: function(data) {
						...
						// if (keyword != '') {
							...
							if ( res.length > 0 ) {
								...
								$('.custom-autocomplete-suggestions').find('ul').append('<li class="sortby-sec cust_but customize"><a data-target="#customize-it" data-toggle="modal" href="#">Can\'t find it ? <u>See More Options</u></a></li>');
