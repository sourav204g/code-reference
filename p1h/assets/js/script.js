$(document).on('ready',function(){
    ...
    /* questions Popup */
    $('.questions-popup').on('click', function(){
        $('.questions-popup-box').fadeIn('fast');
        $('html').addClass('no-scroll');
    });
    $('.close-popup').on('click', function(){
        $('.questions-popup-box').fadeOut('fast');
        $('html').removeClass('no-scroll');
    });
    
    /* schedule Popup */
    $('.schedule-popup').on('click', function(){
        $('.schedule-popup-box').fadeIn('fast');
        $('html').addClass('no-scroll');
    });
    $('.close-popup').on('click', function(){
        $('.schedule-popup-box').fadeOut('fast');
        $('html').removeClass('no-scroll');
    });

    /* Signup Popup */
     /*   $('.customize').on('click', function(){
            $('.customize-box').fadeIn('fast');
            $('html').addClass('no-scroll');
        });
        $('.close-popup').on('click', function(){
            $('.customize-box').fadeOut('fast');
            $('html').removeClass('no-scroll');
        });*/
    ...
    $('#toggle-widget h2').removeClass('active');
    $('#toggle-widget .content').hide();
    //$('#toggle-widget h2:first').addClass('active').next().slideUp('fast');
...
/*Product Page sldebar in Desktop View*/
    
   /* $('#toggle-cat-widget .content').hide();
    $('#toggle-cat-widget h2:first').addClass('active').next().slideDown('fast');
    $('#toggle-cat-widget h2').on("click", function(){
    if($(this).next().is(':hidden')) {
    $('#toggle-cat-widget h2').removeClass('active').next().slideUp('fast');
    $(this).toggleClass('active').next().slideDown('fast');
    }
    }); */

/*Product Page sldebar in mobile view*/

// if ($(window).width() < 560) {
            $('.tax-service_categories #toggle-cat-widget h2').removeClass('active');
            $('.tax-service_categories #toggle-cat-widget .content').hide();
            
            $('.tax-service_categories #toggle-cat-widget h2').on("click", function(){
            if($(this).next().is(':hidden')) {
            $('.tax-service_categories #toggle-cat-widget h2').removeClass('active').next().slideUp('fast');
            $(this).toggleClass('active').next().slideDown('fast');
            }
            });
// }
